...
local args = {
	"cmd",
	"-gh 110547096233249 110547096233249 110547096233249"
}
...
if not isfolder("ToryAssets/Hacker-x") then
    makefolder("ToryAssets/Hacker-x")
end

local MusicAssets = {
    {name = "ToryAssets/Hacker-x/Welcome to the Game - Main Menu.mp3", url = "https://raw.githubusercontent.com/Jskfhggjxu/hacker-x-assets/main/Welcome%20to%20the%20Game%20-%20Main%20Menu.mp3"},
    {name = "ToryAssets/Hacker-x/Mayhem-DoUKnoTechKno.mp3", url = "https://raw.githubusercontent.com/Jskfhggjxu/hacker-x-assets/main/Mayhem-DoUKnoTechKno.mp3"},
    {name = "ToryAssets/Hacker-x/Clubbed.mp3", url = "https://raw.githubusercontent.com/Jskfhggjxu/hacker-x-assets/main/Clubbed.mp3"},
}

for _, asset in ipairs(MusicAssets) do
    if not isfile(asset.name) then
        ...
        if success and content and #content > 0 then
            ...
            warn("download failed:" .. asset.name)
        end
    end
end
...
local MusicFileMap = {
    [2007066385] = "ToryAssets/Hacker-x/Welcome to the Game - Main Menu.mp3",
    [170282324] = "ToryAssets/Hacker-x/Mayhem-DoUKnoTechKno.mp3",
    [1837185092] = "ToryAssets/Hacker-x/Clubbed.mp3",
}
local SoundIdCache = {}
local function getMusicSoundId(songId)
    if SoundIdCache[songId] then
        return SoundIdCache[songId]
    end
    local result = "rbxassetid://" .. songId
    local path = MusicFileMap[songId]
    if path and isfile(path) then
        local ok, custom = pcall(getcustomasset, path)
        if ok and type(custom) == "string" and custom ~= "" then
            result = custom
        end
    end
    SoundIdCache[songId] = result
    return result
end
----------------------------------------------------------------------------------
...
function ScreenAway()
  ...
  Cso("1184953203", tors, 3, 1)
    ...
    Screen1.Transparency = 1.5
    Screen2.Transparency = 1.5
    Screen3.Transparency = 1.5
	Speed = 16
	Screen = false
	movelegs = false
	attack = false
...
--[[			SONG = 1825107283
			...
			game:GetService("StarterGui"):SetCore("SendNotification", {
                Title = "Warning",
                Text = "2 mode music id got deleted"
            })
		elseif key == "3" then
--[[			SONG = 1366716306
			Music.TimePosition = 0
			...
			game:GetService("StarterGui"):SetCore("SendNotification", {
                Title = "Warning",
                Text = "3 mode music id got deleted"
            })
		elseif key == "4" then
			SONG = 170282324
		    Music.TimePosition = 0
...
--[[			SONG = 200602561
			...
			game:GetService("StarterGui"):SetCore("SendNotification", {
                Title = "Warning",
                Text = "6 mode music id got deleted"
            })
		end
	end
end)
...
task.spawn(function()
    ...
    local TEXTURE_ID = "106203239820737"
    local ANCHOR_HANDLE = false

    local NetlessConfig = {
        { PartName = "part1", HideThres = 0.99, Offset = CFrame.new(0, 0, 0) * CFrame.Angles(math.rad(0), math.rad(-180), math.rad(0)) },
        { PartName = "part2", HideThres = 0.99, Offset = CFrame.new(0, 0, 0) * CFrame.Angles(math.rad(0), math.rad(-180), math.rad(0)) },
        { PartName = "part3", HideThres = 0.99, Offset = CFrame.new(0, 0, 0) * CFrame.Angles(math.rad(0), math.rad(-180), math.rad(0)) },
    }

    local function autoRenameScreens()
        local screens = {}
        for _, v in pairs(Character:GetChildren()) do
            if v:IsA("BasePart") and v.Name == "Part" then
                local w = v:FindFirstChildOfClass("Weld")
                if w and w.Part0 == HRP then
                    table.insert(screens, { Part = v, X = w.C0.Position.X })
                end
            end
        end
        table.sort(screens, function(a, b) return a.X < b.X end)
        for i, s in ipairs(screens) do
            if i <= 3 then s.Part.Name = ("part" .. i) end
        end
    end
    autoRenameScreens()
    ...
    local usedHandles = {}
    local function takeUnusedHandle(textureId)
        for _, acc in pairs(Character:GetChildren()) do
            if acc:IsA("Accessory") then
                local handle = acc:FindFirstChild("Handle")
                if handle then
                    local mesh = handle:FindFirstChildOfClass("SpecialMesh")
                    local currentId = handle:IsA("MeshPart") and handle.TextureID
                        or (mesh and mesh.TextureId or "")
                    if string.find(tostring(currentId), textureId) and not usedHandles[handle] then
    ...
    for _, acc in pairs(Character:GetChildren()) do
        if acc:IsA("Accessory") then
            ...
            if h then
                ...
                local id = h:IsA("MeshPart") and h.TextureID or (m and m.TextureId or "")
                if string.find(tostring(id), TEXTURE_ID) then
                    n = n + 1
                end
            end
        end
    ...
    local function setupSwordSafety(handle)
        ...
        if ANCHOR_HANDLE then handle.Anchored = true end
    ...
    local waitTime = 0
    while #ActiveSyncList < #NetlessConfig and waitTime < 30 do
        for _, config in pairs(NetlessConfig) do
            if not config.Activated then
                local part = findPartByName(config.PartName)
                local handle = part and takeUnusedHandle(TEXTURE_ID)
                if part and handle then
                    handle:BreakJoints()
                    setupSwordSafety(handle)
                    config.Activated = true
                    table.insert(ActiveSyncList, {
                        HatHandle = handle,
                        TargetPart = part,
                        Offset = config.Offset,
                        HideThres = config.HideThres,
                    })
                end
            end
        end
    ...
    if #ActiveSyncList < #NetlessConfig then
        return
    end
    ...
    RunService.Heartbeat:Connect(function()
        ...
        for _, syncData in pairs(ActiveSyncList) do
            ...
            if hat and hat.Parent and target and target.Parent then
                local targetCF
                if target.Transparency >= syncData.HideThres then
                    local p = HRP.Position
                    targetCF = CFrame.new(p.X, p.Y - 20, p.Z)
                else
                    targetCF = target.CFrame * syncData.Offset
                end
                hat.CFrame = targetCF
...
while true do
	...
	hum.Name = "HUM"
	hum.WalkSpeed = Speed
	Music.SoundId = getMusicSoundId(SONG)
	Music.Looped = true
	Music.Pitch = 1
	Music.Volume = 2
	Music.Parent = tors
