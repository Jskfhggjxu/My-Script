...
local page = window:AddPage({Title = "绕过机制"})
if deleted then
...
local page = window:AddPage({Title = "透视"})

local activated = false
page:AddToggle({Caption = "透视彩虹变化(可能会导致卡顿)", Default = false, Callback = function(b)
...
for i, v in vals.ESP do 
    local displayName = espTypeMap[i] or i:gsub("ESP", ""):gsub("  ", " ") 
    page:AddToggle({
        Caption = displayName,
        Default = v,
        Callback = function(b)
