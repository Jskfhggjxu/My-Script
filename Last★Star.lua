local args = {
	"cmd",
	"-gh 5316479641 5316539421 5699795428 5316549755 5268602207 5268555719"
}
...
-- script = GetInstanceFromScript("rbxassetid://11225944180","LastStar","Script")
-- you have a backup ID if this id got http 404
script = GetInstanceFromScript("rbxassetid://17346979612","LastStar","Script")
local fury = script.Values:WaitForChild("Fury")
...
DangerRedile - Convert & More modes
...
if x then x.Disabled=true x:Remove() end
if y then y:Remove() end
local nametodisplay = string.upper(game:GetService("Players").LocalPlayer.DisplayName)
...
if not workspace:FindFirstChild("LastStarMouseIgnore") then
	script:FindFirstChild("LastStarMouseIgnore").Parent = workspace
else
	script:FindFirstChild("LastStarMouseIgnore"):Destroy()
end
...
function chatmsg(msg)
	task.spawn(function()
		if plr then
			local head,laststar = plr:FindFirstChild("Head"),script
				...
				if CG then
					...
					if chattype == "Typewriter" then
						for i = 1, #msg do
							...
							local s = script.Sounds.ChatSound:Clone()
							s.Parent = head
							s.Playing=true
...
function camshake(pos,power,time)
    for index, child in pairs(workspace:GetDescendants()) do
		if child:FindFirstChildOfClass("Humanoid") then
			...
			if root and game.Players:GetPlayerFromCharacter(child) then
				if (root.Position - pos).Magnitude <= 35 then
					local timer = math.ceil(time) or 35
                    local power = math.ceil(power) or 5
                    if hum then
	                    local fader = power/timer
	                    for i = 1, timer do
		                    wait(1/120)
		                    Hum.CameraOffset = V3(rand(-(power-(fader*i)),(SHAKE-(FADER*i)))/10,rand(-(power-(fader*i)),(power-(fader*i)))/10,rand(-(power-(fader*i)),(power-(fader*i)))/10)
	                    end
	                    Hum.CameraOffset = V3()
                    end
				end
...
chatmsg("Last Star convert/modded by Tory")
...
function glitchscreen(time,color)
	task.spawn(function()
		local images = {18328673765,15988110169,15988121014,15988137121,17219734815,8277325490,8277320341,17219756536,13729435867,9484176300,8277321458,15826350380,5197343873}
		
		local GlitchHolder = Instance.new("ImageLabel",gui)
		
		GlitchHolder.ZIndex = math.random(-5,5)
		GlitchHolder.Size = UDim2.new(1.5, 0, 1.5, 0)
		GlitchHolder.Position = UDim2.new(-0.25, 0, -0.25, 0)
		GlitchHolder.ImageColor3 = color
		GlitchHolder.Image = "rbxassetid://"..images[math.random(1,#images)]
		GlitchHolder.BackgroundTransparency = 1
		if math.random(1,2)==1 then
			GlitchHolder.ImageRectOffset = Vector2.new(1024, 0)
			GlitchHolder.ImageRectSize = Vector2.new(-1024, 1024)
		end
		
		for i = 1, time do
			GlitchHolder.Image = "rbxassetid://"..images[math.random(1,#images)]
			GlitchHolder.ImageTransparency = math.random(100,350)/1000
			GlitchHolder.Rotation = math.random(-2500,2500)/1000
			task.wait(1/120)
		end
		GlitchHolder:Destroy()
	end)
end
...
function formswitch(textcolor,textstrokecolor,theme,primary,secondary,walkspeed,jumppower,attack,formname,formfont,wingstyle,formidle,themevolume,fancyexplosionbool,altindexnumber,corebool,scriptname,dynamic,themename,extrawingsbool,marbleimage)
	...
	if script.Values.CurrentForm.Value:find("SWITCH") then return end
	Root.Anchored=true
	Root.Velocity = V3()
	...
	elseif typeof(theme) == "string" then
		pcall(function() Theme.SoundId = getcustomasset(theme) end)
	end
	...
	if fury.Value == true then
		...
		Theme.Volume = 1.5
		themename = "Big-Booty-Latinas \|/ (in your area)"
	end
...
function cooldown(duration)
	if cds == true and fury.Value == false then
...
function apexslam()
	...
	wait(.1)
	if target then
		if target.Humanoid.Health > target.Humanoid.MaxHealth*.3 and fury.Value == false and instakill == false then
			target = nil
...
local apexlines = {"Waste of Space.","Get out of my way.","Imma kill you lol"}
...
local warpspeedlines = {"Untouchable!.","I saw Epic around here somewhere.","Demolition!"}
local intricacylines = {"Complexity at its finest.","This ends now.","Face your fears."}
local OHGODWHYHELPMESOMEBODYPLEASE = {"Secret mode.","cum or else...","Is anyone actually going to see this?"}
-- // BASE FORMS
...
local changewheeltable = {
	...
	["EXECUTIONALTS"]=function() changewheel(true,"EXECUTION","SPECTRAFIELD","PROHIBITION","ĂÂ£TERNITY","NEBULA","AURORA","UTTERANCE","SOMNAMBULIST","","","Bodoni","Arcade","ArialBold","SourceSansSemibold","SciFi","GothamBlack","Fondamento","Bodoni","Arial","Arial",c3(255,255,255),c3(255,255,255),c3(45,45,45),c3(0,0,0),c3(255,0,0),c3(80,0,0),c3(255,255,255),c3(200,200,200),c3(210,100,255),c3(100,160,255),c3(130, 200, 255),c3(255,255,255),c3(240,0,40),c3(10,10,10),c3(166, 197, 255),c3(208, 189, 255),c3(20,20,20),c3(20,20,20),c3(20,20,20),c3(20,20,20)) returnwheel = "BASE" end;
...
local apexalttable = {
	["APEX"]=function()
		...
		if x > 1 then
			...
			chatmsg("MelonSkidder and Maddox are fat skids")
		end
	end;
	...
	["AGONY"]=function()  formswitch(C3R(0,0,0),C3R(200,0,0),8703175543,C3R(200,0,0),C3R(0,0,0),14,45,1.1,"AGONY","Antique","APEX","APEX",0.25,false,2,false,"LĂâ€PSĂÂ£",false,"-| D o o M |-") end;
...
local repressionalttable = {
	...
	["ECLIPSE"]=function()
		local t = {
			function()formswitch(C3R(0,0,0),C3R(255,80,0),1837301451,C3R(255,80,0),C3R(0,0,0),16,50,1.1,"ECLIPSE","Fantasy","REPRESSION","REPRESSION",0.3,false,3,false,"LĂâ€PSĂÂ£",false,"gmtn. (witch's slave) // Furioso Melodia")end;
			function()formswitch(C3R(0,0,0),C3R(0,130,255),142376088,C3R(0,130,255),C3R(0,0,0),16,50,1.15,"DARKENING","Highway","REPRESSION","REPRESSION",0.3,false,3,false,"LĂâ€PSĂÂ£",false,"gmtn. (witch's slave) // Furioso Melodia (2017 VIP)")end
		}
...
local executionalttable = {
	...
	["ĂÂ£TERNITY"] = function() formswitch(C3R(200,200,200),C3R(255,255,255),7026389855,C3R(255,255,255),C3R(150,150,150),200,50,0.9,"ĂÂ£TERNITY","SourceSansSemibold","EXECUTION","EXECUTION",0.3,false,-1,false,"LAST★STAR",false,"Egipt Rad B)") end;
	["NEBULA"] = function() formswitch(C3R(210,100,255),C3R(100,160,255),5410080857,C3R(210,100,255),C3R(100,160,255),500,100,0.6,"NEBULA","SciFi","EXECUTION","EXECUTION",0.3,false,5,false,"LĂâ€PSĂÂ£",true,"Who's laughing now bastard") end;
	["AURORA"] = function() formswitch(C3R(255,255,255),C3R(100,160,255),7024332460,script.Dynamics.Aurora.Value,C3R(255,255,255),500,100,0.6,"AURORA","GothamBlack","EXECUTION","EXECUTION",0.3,false,6,false,"LĂâ€PSĂÂ£",true,"Pegboard Nerds - Shaku |Nothing more to say|") end;
...
local anarchyalttable = {
	...
	["LUNAR"]=function() formswitch(C3R(0,0,0),C3R(0,0,255),4747259294,C3R(0,0,255),C3R(0,0,0),80,60,1.1,"LUNAR","Bodoni","ANARCHY","ANARCHY",0.3,false,3,false,"LĂâ€PSĂÂ£",false,"Lchavasse // Space Abyss") end;
...
local virtuealttable = {
	...
	["DIAPHANOUS"]=function() formswitch(C3R(150,150,150),C3R(255,255,255),586732256,C3R(200,200,200),C3R(120,120,120),60,40,1.25,"DIAPHANOUS","Fantasy","VIRTUE","VIRTUE",0.4,false,1,false,"LĂâ€PSĂÂ£",false,"xi // Happy End of the World") end;
	...
	["ENIGMA"]=function() formswitch(C3R(0,0,0),C3R(0,130,255),1747455288,C3R(0,110,255),C3R(0,0,0),70,50,0.8,"ENIGMA","SciFi","VIRTUE","VIRTUE",0.4,false,3,false,"LĂâ€PSĂÂ£",false,"XI // Ascension to Heaven") end;
	...
	["HEARTFELT"]=function() formswitch(C3R(35,14,57),C3R(127,96,170),5712004125,C3R(35,14,57),C3R(127,96,170),51,50,1,"HEARTFELT","Nunito","VIRTUE","VIRTUE",0.4,false,3,false,"CHROMOSPHERIC",false,"Cororo // Ă£â€Â¦Ă£â€Â¨Ă£Æ’Â³Ă£Æ’Â¬Ă£Æ’Â©Ă£ÂÂ®Ă¦Â°Â·Ă¨ÂÂ¯") end;
...
local dissonancealttable = {
	...
	["DISTORTION"]=function() formswitch(C3R(255,0,0),C3R(0,0,0),3108672499,C3R(255,0,0),C3R(0,0,0),60,40,1.1,"DISTORTION","SciFi","DISSONANCE","DISSONANCE",0.35,false,1,false,"LĂâ€PSĂÂ£",true,"Kobaryo // Necrophagiatoast") end;
	["ALTERATION"]=function() formswitch(C3R(255,255,255),C3R(0,0,0),833792144,C3R(255,255,255),C3R(0,0,0),130,30,0.8,"ALTERATION","Bodoni","DISSONANCE","DISSONANCE",0.4,false,2,false,"LĂâ€PSĂÂ£",true,"The Quick Brown Fox - PROSELYTISM") end
}
local nihilalttable = {
	...
	["TERMINATION"]=function() formswitch(C3R(0,0,0),C3R(0,0,0),6807085919,C3R(0,255,0),C3R(255,255,255),300,60,0.7,"TERMINATION","SciFi","NIHIL","NIHIL",0.35,false,-1,false,"LĂâ€PSĂÂ£",true,"Renard & Adraen // Terminal") end;
...
script.Events.Switching.OnServerEvent:Connect(function(player,form)
	...
	elseif form == "VICISSITUDE" then
		formswitch(C3R(61, 16, 20),C3R(175, 47, 56),2617725119,C3R(175, 47, 56),C3R(61, 16, 20),600,75,1,"VICISSITUDE","Garamond","ABSOLUTION","ABSOLUTION",.4,false,1,false,"True Star Glitcher",false,"DJ Grimoire // Noisy Minority")
	...
	elseif form == "ACQUITTAL" then
		formswitch(C3R(100,35,35),C3R(155,30,30),6992712759,C3R(155,30,30),C3R(100,35,35),500,150,1,"ACQUITTAL","Nunito","ABSOLUTION","ABSOLUTION",3,false,10,false,"True Star Glitcher",false,"	RedOgre, OZIGIRI, DJ Myosuke - Positron Emission")
	end
	...
	if form == "INFERNUM" and altwheel == true and altswitchcooldown == 0 then
		formswitch(C3R(0,0,0),C3R(255,70,0),3619670346,C3R(255,70,0),C3R(0,0,0),14,50,1,"INFERNUM","JosefinSans","INFERNUM","INFERNUM",.45,false,0,false,"True Star Glitcher",false,"Dubstep Hero")
	elseif form == "HELLFIRE" then
		local x = rand(1,5)
		if x ~= 1 then
			formswitch(C3R(0,0,0),C3R(70,200,255),4620556828,C3R(50,150,255),C3R(0,0,0),20,50,.95,"HELLFIRE","Arcade","INFERNUM","INFERNUM",.4,false,1,false,"True Star Glitcher",false,"JAKAZiD - Make Me Burn")
		else
	...
	elseif form == "INCANDESCENCE" then
		formswitch(C3R(255, 147, 125),C3R(255, 224, 128),6430675969,C3R(255, 224, 128),C3R(255, 147, 125),60,50,.7,"INCANDESCENCE","Kalam","INFERNUM","INFERNUM",.35,false,2,false,"True Star Glitcher",false,"Mr. Ivex // Thinking")
	elseif form == "FERVENCY" then
		formswitch(C3R(97, 100, 255),C3R(144, 150, 255),4899357897,C3R(144, 150, 255),C3R(97, 100, 255),20,50,1,"FERVENCY","Fondamento","INFERNUM","INFERNUM",.4,false,2,false,"True Star Glitcher",false,"onumi // ARROGANCE")
	...
	if form == "WARPSPEED" and altwheel == true and altswitchcooldown == 0 then
		formswitch(C3R(70, 181, 255),C3R(208, 241, 255),1438843849,C3R(151, 165, 255),C3R(64, 121, 255),400,50,1,"WARPSPEED","Michroma","WARPSPEED","WARPSPEED",.4,false,0,false,"True Star Glitcher",false,"100% S P E E D")
	...
	if state == "on" then
		...
		elseif key == Enum.KeyCode.M then
			...
			elseif script.Values.CurrentForm.Value == "SAGITTARIUS" then
				...
				if fury.Value == false then Theme.TimePosition = 0 end
			end
		end
...
script:WaitForChild("Events"):WaitForChild("TweenRoot").OnClientEvent:Connect(function(tim,style,direction,prop)
	...
	local tween = tweens:Create(plr:WaitForChild("HumanoidRootPart"),info,prop)
	tween:Play()
end)
...
info = TweenInfo.new(.2,Enum.EasingStyle.Sine,Enum.EasingDirection.Out,0,false,0)
info2 = TweenInfo.new(.2,Enum.EasingStyle.Sine,Enum.EasingDirection.In,0,false,0)
prop1,prop2 = {Value=1},{Value=0}
tween1,tween2 = tweens:Create(gui.WheelSize,info,prop1),tweens:Create(gui.WheelSize,info2,prop2)
...
sliceinfo = TweenInfo.new(.1,Enum.EasingStyle.Sine,Enum.EasingDirection.InOut,0,false,0)
p1,p2 = {Size=UDim2.new(1.1,0,1.1,0)},{Size=UDim2.new(1,0,1,0)}
p3,p4 = {Size=UDim2.new(.1,0,.1,0)},{Size=UDim2.new(.07,0,.07,0)}

hinfo = TweenInfo.new(.4,Enum.EasingStyle.Circular,Enum.EasingDirection.Out,0,false,0)
switchedcolour = Color3.fromRGB(0,0,0)
...
button = gui.FormSelection.Shade.Switch.Button
...
script:WaitForChild("Values"):WaitForChild("CurrentForm").Changed:Connect(function()
	...
	tweens:Create(gui.SwitchShine,TweenInfo.new(.5,Enum.EasingStyle.Circular,Enum.EasingDirection.Out,0,false,0),{BackgroundTransparency=1}):Play()
end)
...
heartbeat:connect(function()
	...
	upperbaroffset += 2
	if gui:FindFirstChild("UpperBar"):FindFirstChild("Labeller") then
	...
	gui.UpperBar.Labeller.Text = "|╫|  LAST ★ STAR // "..script.Values.CurrentForm.Value.." // "..nametodisplay.."  |╫|            "
	...
	if plr:FindFirstChild("Humanoid") then
		gui.Stats.Jump.Text = "JP / "..plr.Humanoid.JumpPower
		gui.Stats.Speed.Text = "SP / "..plr.Humanoid.WalkSpeed
	...
	if workspace:FindFirstChild("Chaos Spire") then
		...
		local t = {"YOU ARE NOT WORTHY.","ARE YOU WORTHY?","YOU ARE NOT WORTHY?","? ? ?","I C U","GET OUT","YOU ARE NOT","YOU ARE NOT WELCOME HERE."}
	...
	for i,v in pairs(gui.FormSelection:GetDescendants()) do
		if v:IsA("TextButton") and v.Parent.Name ~= "Switch" then
			...
			elseif v.Text == "CHAOS" then
				...
				v.Parent.Marble.Inner.ImageColor3 = s
				v.Parent.Marble.Inner.Pattern.ImageColor3 = s
			elseif v.Parent.ID.Value == "GLITCH" then
				p = Color3.fromRGB(rand(0,55),rand(0,55),rand(0,55))
				s = BrickColor.Random().Color
				v.TextStrokeColor3 = p
				v.TextColor3 = s
				v.Parent.ImageColor3 = p
				v.Parent.Marble.ImageColor3 = p
				v.Parent.Marble.Inner.ImageColor3 = s
				v.Parent.Marble.Inner.Pattern.ImageColor3 = s
				local g,l,i,t,c,h = {"G","g"},{"L","l"},{"I","i"},{"T","t"},{"C","c"},{"H","h"}
				v.Text = g[math.random(1,2)]..l[math.random(1,2)]..i[math.random(1,2)]..t[math.random(1,2)]..c[math.random(1,2)]..h[math.random(1,2)]
				local ft = {"Arcade","Antique","SciFi","Bodoni","Garamond","Oswald","Arial","Bangers","Cartoon","Code","Creepster","DenkOne","Fantasy","Fondamento","FredokaOne","GothamSemibold","GrenzeGotisch","Highway","IndieFlower","JosefinSans","Jura","Kalam","LuckiestGuy","Merriweather","Michroma","Nunito","PatrickHand","PermanentMarker","RobotoMono","Sarpanch","SourceSansSemibold","SpecialElite","Ubuntu","TitilliumWeb"}
				v.Font = ft[math.random(1,#ft)]
			elseif v.Text == "NHELV" then
				local bc = RVAL({C3R(math.random(200,255),math.random(200,255),math.random(200,255)),C3R(math.random(150,255),math.random(0,10),math.random(0,10)),C3R(50,50,50)})
				p = Color3.new(bc.R,bc.G,bc.B)
				s = Color3.fromRGB(1-bc.R,1-bc.G,1-bc.B)
				v.TextStrokeColor3 = p
				v.TextColor3 = s
				v.Parent.ImageColor3 = p
				v.Parent.Marble.ImageColor3 = p
				v.Parent.Marble.Inner.ImageColor3 = s
				v.Parent.Marble.Inner.Pattern.ImageColor3 = s
			elseif v.Text == "SPAMINAUCINIHIPHICATIOPHOBIA" then
				local bc = C3R(255*math.sin(sine),255*math.sin(sine),255*math.sin(sine))
				p = Color3.new(bc.R,bc.G,bc.B)
				s = Color3.fromRGB(1-bc.R,1-bc.G,1-bc.B)
				v.TextStrokeColor3 = p
				v.TextColor3 = s
				...
				v.Parent.Marble.Inner.ImageColor3 = s
				v.Parent.Marble.Inner.Pattern.ImageColor3 = s
			elseif v.Text == "BYE BYE" then
				local bc = RVAL({C3R(math.random(200,255),math.random(200,255),math.random(200,255)),C3R(0,0,math.random(150,225)),C3R(0,0,0)})
				p = Color3.new(bc.R,bc.G,bc.B)
				s = Color3.fromRGB(1-bc.R,1-bc.G,1-bc.B)
				v.TextStrokeColor3 = p
				v.TextColor3 = s
			...
			elseif v.Text == "KARMA" then
				...
				v.Parent.Marble.Inner.ImageColor3 = s
				v.Parent.Marble.Inner.Pattern.ImageColor3 = s
			elseif v.Parent.ID.Value == "RAVAGEON" then
				if script.Values.CurrentForm.Value == "RAVAGEON" then
					if theme.TimePosition < 26.7 then
						p = C3R(100+math.abs(155*math.sin(sine/25)),100+math.abs(155*math.sin(sine/25)),100+math.abs(155*math.sin(sine/25)))
						s = C3R(1-p.R,1-p.G,1-p.B)
					elseif theme.TimePosition >= 26.7 and theme.TimePosition < 28.2 then
						p = C3R(math.random(225,255),100,math.random(200,255))
						s = C3R(math.random(225,255),100,math.random(200,255))
					elseif theme.TimePosition >= 28.2 and theme.TimePosition < 29.25 then
						p = C3R(math.random(189,255),0,math.random(174,255))
						s = C3R(math.random(189,255),0,math.random(174,255))
					elseif theme.TimePosition >= 29.25 and theme.TimePosition < 37.9 then
						p = C3R(100+math.abs(155*math.sin(sine/25)),100+math.abs(155*math.sin(sine/25)),100+math.abs(155*math.sin(sine/25)))
						s = C3R(1-p.R,1-p.G,1-p.B)
					elseif theme.TimePosition >= 37.9 and theme.TimePosition < 43 then
						local lol,lmao = math.random(0,255),math.random(0,255)
						p = C3R(lol,lol,lol)
						s = C3R(lmao,lmao,lmao)
					elseif theme.TimePosition >= 43 and theme.TimePosition < 43.8 then
						p = C3R(0,0,0)
						s = C3R(0,0,0)
					elseif theme.TimePosition >= 43.8 and theme.TimePosition < 111 then
						p = RVAL({C3R(math.random(20,70),math.random(20,70),math.random(20,70)),C3R(math.random(125,255),math.random(0,48),math.random(117,255))})
						s = C3R(115+math.abs(145*math.sin(sine/30)),0,50+math.abs(175*math.sin(sine/30)))
					elseif theme.TimePosition >= 111 and theme.TimePosition < 113.26 then
						p = C3R(math.random(0,100),0,0)
						s = C3R(math.random(0,50),0,0)
					elseif theme.TimePosition >= 113.26 and theme.TimePosition < 119.5 then
						p = C3R(math.random(10,255),0,0)
						s = C3R(math.random(10,255),0,0)
					elseif theme.TimePosition >= 119.5 and theme.TimePosition < 154.5 then
						p = RVAL({C3R(math.random(20,70),math.random(20,70),math.random(20,70)),C3R(math.random(125,255),math.random(0,48),math.random(117,255))})
						s = C3R(115+math.abs(145*math.sin(sine/30)),0,50+math.abs(175*math.sin(sine/30)))
					else
						local lol,lmao = math.random(0,100),math.random(0,100)
						p = C3R(lol,lol,lol)
						s = C3R(lmao,lmao,lmao)
					end
				else				
					p = C3R(150+math.abs(65*math.sin(sine/10)),0,154+math.abs(50*math.sin(sine/10)))
					s = RVAL({C3R(255,100,204),C3R(50,50,50),C3R(100,100,100),C3R(175,175,175),C3R(150+math.abs(105*math.sin(sine/10)),0,154+math.abs(100*math.sin(sine/10)))})
				end
				v.TextStrokeColor3 = p
				v.TextColor3 = s
				v.Parent.ImageColor3 = p
				...
				v.Parent.Marble.Inner.ImageColor3 = s
				v.Parent.Marble.Inner.Pattern.ImageColor3 = s
				local R,A,V,G,E,O,N = {"R","r"},{"A","a"},{"V","v"},{"G","g"},{"E","e"},{"O","o"},{"N","n"}
				v.Text = R[math.random(1,2)]..A[math.random(1,2)]..V[math.random(1,2)]..A[math.random(1,2)]..G[math.random(1,2)]..E[math.random(1,2)]..O[math.random(1,2)]..N[math.random(1,2)]
				if math.random(1,4)==1 then
					local texttable = {"SICK","HAHA","AAAAA","FIX THIS MIND","FREAK","SOULLESS","MENTAL","EPIC WAS HERE","DIE","END"}
					v.Text = texttable[math.random(1,#texttable)]
				end
			elseif v.Text == "ECHO" then
				p = Color3.fromRGB(theme.PlaybackLoudness/2,0,0)
				s = Color3.fromRGB(155,0,0)
				v.TextStrokeColor3 = p
				v.TextColor3 = s
				v.Parent.ImageColor3 = p
				v.Parent.Marble.ImageColor3 = p
	...
	if activedynamic == true then
		...
		elseif script.Values.CurrentForm.Value == "GLITCH" then
			local g,l,i,t,c,h = {"G","g"},{"L","l"},{"I","i"},{"T","t"},{"C","c"},{"H","h"}
			NG.LowerHalf.FormName.Text = g[rand(1,2)]..l[rand(1,2)]..i[rand(1,2)]..t[rand(1,2)]..c[rand(1,2)]..h[rand(1,2)]
			local ft = {"Arcade","Antique","SciFi","Bodoni","Garamond","Oswald","Arial","Bangers","Cartoon","Code","Creepster","DenkOne","Fantasy","Fondamento","FredokaOne","GothamSemibold","GrenzeGotisch","Highway","IndieFlower","JosefinSans","Jura","Kalam","LuckiestGuy","Merriweather","Michroma","Nunito","PatrickHand","PermanentMarker","RobotoMono","Sarpanch","SourceSansSemibold","SpecialElite","Ubuntu","TitilliumWeb"}
			NG.LowerHalf.FormName.Font = ft[rand(1,#ft)]
			p = C3R(rand(0,55),rand(0,55),rand(0,55))
			s = BrickColor.Random().Color
			local chartable = {"A","a","B","b","C","c","D","d","E","e","F","f","G","g","H","h","I","i","J","j","K","k","L","l","M","m","N","n","O","o","P","p","Q","q","R","r","S","s","T","t","U","u","V","v","W","w","X","x","Y","y","Z","z","1","!","2","@","3","#","4","$","5","%","6","^","7","&","8","*","9","(","0",")","[","]","{","}",":",";","'",'"',"|",",","<",".",">","/","?","_"}
			NG.LowerHalf.ScriptName.Text = chartable[rand(1,#chartable)]..chartable[rand(1,#chartable)]..chartable[rand(1,#chartable)]..chartable[rand(1,#chartable)]..chartable[rand(1,#chartable)]..chartable[rand(1,#chartable)]..chartable[rand(1,#chartable)]..chartable[rand(1,#chartable)]..chartable[rand(1,#chartable)]..chartable[rand(1,#chartable)]..chartable[rand(1,#chartable)]..chartable[rand(1,#chartable)].."★STAR // "..chartable[rand(1,#chartable)]..chartable[rand(1,#chartable)]..chartable[rand(1,#chartable)]..chartable[rand(1,#chartable)]..chartable[rand(1,#chartable)]..chartable[rand(1,#chartable)]..chartable[rand(1,#chartable)]
		elseif script.Values.CurrentForm.Value == "RAVAGEON" then
			if theme.TimePosition < 26.7 then
				p = C3R(100+math.abs(155*math.sin(sine/25)),100+math.abs(155*math.sin(sine/25)),100+math.abs(155*math.sin(sine/25)))
				s = C3R(1-p.R,1-p.G,1-p.B)
			elseif theme.TimePosition >= 26.7 and theme.TimePosition < 28.2 then
			...
			elseif theme.TimePosition >= 113.26 and theme.TimePosition < 119.5 then
				...
				s = C3R(math.random(10,255),0,0)
				for i = 1, 2 do
					glitchscreen(1,C3R(math.random(10,255),0,0))
				end
			elseif theme.TimePosition >= 119.5 and theme.TimePosition < 154.5 then
				p = RVAL({C3R(math.random(20,70),math.random(20,70),math.random(20,70)),C3R(math.random(125,255),math.random(0,48),math.random(117,255))})
				s = C3R(115+math.abs(145*math.sin(sine/30)),0,50+math.abs(175*math.sin(sine/30)))
			...
			local R,A,V,G,E,O,N = {"R","r"},{"A","a"},{"V","v"},{"G","g"},{"E","e"},{"O","o"},{"N","n"}
			NG.LowerHalf.FormName.Text = R[math.random(1,2)]..A[math.random(1,2)]..V[math.random(1,2)]..A[math.random(1,2)]..G[math.random(1,2)]..E[math.random(1,2)]..O[math.random(1,2)]..N[math.random(1,2)]
			if math.random(1,4)==1 then
				local texttable = {"SICK","HAHA","AAAAA","FIX THIS MIND","FREAK","SOULLESS","MENTAL","EPIC WAS HERE","DIE","END"}
				NG.LowerHalf.FormName.Text = texttable[math.random(1,#texttable)]
			end
		...
		for i,v in pairs({NG.LowerHalf.Vis1,NG.LowerHalf.Vis2,NG.LowerHalf.Vis3,NG.LowerHalf.Vis4,NG.LowerHalf.Vis5,NG.LowerHalf.Vis6,NG.LowerHalf.Vis7,NG.LowerHalf.Vis8,NG.LowerHalf.Vis9}) do
			pcall(function() v.UIGradient.Color = ColorSequence.new({ColorSequenceKeypoint.new(0,s),ColorSequenceKeypoint.new(1,p)}) end)
		end
		...
		for i,v in pairs({wing1}) do
			...
			elseif script.Values.CurrentForm.Value == "ALTERATION" then
				p = C3H(0,0,rand(1,100)/100)
				s = C3H(0,0,rand(1,100)/100)
			elseif script.Values.CurrentForm.Value == "NHELV" then
				local bc = RVAL({C3R(math.random(200,255),math.random(200,255),math.random(200,255)),C3R(math.random(150,255),math.random(0,10),math.random(0,10)),C3R(50,50,50)})
				p = Color3.new(bc.R,bc.G,bc.B)
				s = Color3.fromRGB(1-bc.R,1-bc.G,1-bc.B)
	...
	if gui:FindFirstChild("MobileSongMute") and gui:FindFirstChild("MobileFormChange") then
		...
		if v then
			if script then
				...
				if Root and Head and wing1 and NG and Hum and secondary and primary then
					for x,z in pairs({"apexauraval","harmonyoffset","infernumdelay","repressionoffset","executionoffset","virtuedelay","flyingdelay","anarchydelay","anarchyoffset"}) do
					...
					if plr then
						if script then
							if script:FindFirstChild("Settings") then
								if script.Settings:WaitForChild("ChatType").Value == "Simple" then
					...
					if not script or enableauras == "All" or enableauras == "You" and plr.Name == plr.Name then
						...
						elseif script.Values.CurrentBaseForm.Value == "HARMONY" then
						...
						elseif script.Values.CurrentBaseForm.Value == "GLITCH" then
							effect("Cube",Torso.CFrame*CFrame.new(math.sin(-2,2)*math.sin(sine/10),18+math.sin(-2,2)*math.sin(sine/10),18+math.sin(-2,2)*math.sin(sine/10))*CFrame.Angles(math.rad(180*math.sin(sine/5)),math.rad(-180*math.sin(sine/5)),math.rad(180*math.sin(sine/5))),Vector3.new(10,10,10),wing1.Primary.Color,Enum.Material.Neon,0.65,0.25,Enum.EasingStyle.Quad,Enum.EasingDirection.Out,false,{Color=wing1.Secondary.Color;Size=Vector3.new(10,10,10)},1,Enum.EasingStyle.Linear)
							effect("Star",Torso.CFrame*CFrame.new(math.sin(-2,2)*math.sin(sine/10),18+math.sin(-2,2)*math.sin(sine/10),18+math.sin(-2,2)*math.sin(sine/10))*CFrame.Angles(math.rad(math.random(-180,180)),math.rad(math.random(-180,180)),math.rad(math.random(-180,180))),Vector3.new(-25,-25,-25),wing1.Primary.Color,Enum.Material.Neon,-999,0.1,Enum.EasingStyle.Quad,Enum.EasingDirection.Out,false,{Size=Vector3.new(10,10,10)},1,Enum.EasingStyle.Linear)
							if theme.PlaybackLoudness > 420 and theme.PlaybackLoudness <= 500 then
								for i = 1, math.random(1,2) do 
									effect("ThinRing",Torso.CFrame*CFrame.new(math.sin(-2,2)*math.sin(sine/10),18+math.sin(-2,2)*math.sin(sine/10),18+math.sin(-2,2)*math.sin(sine/10))*CFrame.Angles(math.rad(math.random(-180,180)),math.rad(math.random(-180,180)),math.rad(math.random(-180,180))),Vector3.new(0,0,0),wing1.Secondary.Color,Enum.Material.Neon,0,.35,Enum.EasingStyle.Bounce,Enum.EasingDirection.Out,false,{Size=Vector3.new(60,1,60)},1,Enum.EasingStyle.Linear)
									effect("Star",Torso.CFrame*CFrame.new(math.sin(-2,2)*math.sin(sine/10),18+math.sin(-2,2)*math.sin(sine/10),18+math.sin(-2,2)*math.sin(sine/10)),Vector3.new(0,0,0),wing1.Primary.Color,Enum.Material.Neon,0,0.4,Enum.EasingStyle.Bounce,Enum.EasingDirection.Out,false,{CFrame=Torso.CFrame*CFrame.new(math.sin(-2,2)*math.sin(sine/10),18+math.sin(-2,2)*math.sin(sine/10),18+math.sin(-2,2)*math.sin(sine/10))*CFrame.new(math.random(-20,20),math.random(-20,20),math.random(-20,20));Size=Vector3.new(8,8,8)},1,Enum.EasingStyle.Linear)
								end
							elseif theme.PlaybackLoudness > 500 then
								for i = 1, math.random(3,7) do 
									local random = math.random(70,85)
									effect("ThinRing",Torso.CFrame*CFrame.new(math.sin(-2,2)*math.sin(sine/10),18+math.sin(-2,2)*math.sin(sine/10),18+math.sin(-2,2)*math.sin(sine/10))*CFrame.Angles(math.rad(math.random(-180,180)),math.rad(math.random(-180,180)),math.rad(math.random(-180,180))),Vector3.new(0,0,0),wing1.Secondary.Color,Enum.Material.Neon,0,.35,Enum.EasingStyle.Bounce,Enum.EasingDirection.Out,false,{Size=Vector3.new(random,1,random)},1,Enum.EasingStyle.Linear)
									effect("Star",Torso.CFrame*CFrame.new(math.sin(-2,2)*math.sin(sine/10),18+math.sin(-2,2)*math.sin(sine/10),18+math.sin(-2,2)*math.sin(sine/10)),Vector3.new(0,0,0),wing1.Primary.Color,Enum.Material.Neon,0,0.4,Enum.EasingStyle.Bounce,Enum.EasingDirection.Out,false,{CFrame=Torso.CFrame*CFrame.new(math.sin(-2,2)*math.sin(sine/10),18+math.sin(-2,2)*math.sin(sine/10),18+math.sin(-2,2)*math.sin(sine/10))*CFrame.new(math.random(-20,20),math.random(-20,20),math.random(-20,20));Size=Vector3.new(10,10,10)},1,Enum.EasingStyle.Linear)
								end
								effect("Cube",Torso.CFrame*CFrame.new(math.sin(-2,2)*math.sin(sine/10),18+math.sin(-2,2)*math.sin(sine/10),18+math.sin(-2,2)*math.sin(sine/10))*CFrame.Angles(math.rad(180*math.sin(sine/5)),math.rad(-180*math.sin(sine/5)),math.rad(180*math.sin(sine/5))),Vector3.new(0,0,0),wing1.Primary.Color,Enum.Material.Neon,0.65,0.25,Enum.EasingStyle.Quad,Enum.EasingDirection.Out,false,{Color=wing1.Secondary.Color;Size=Vector3.new(20,20,20)},1,Enum.EasingStyle.Linear)
								sound(Torso.CFrame*CFrame.new(math.sin(-2,2)*math.sin(sine/10),18+math.sin(-2,2)*math.sin(sine/10),18+math.sin(-2,2)*math.sin(sine/10)),1146698075,1,1,0.1,375)
							end
						elseif script.Values.CurrentBaseForm.Value == "RAVAGEON" then
							local raypart,raypos,normal = ray(Root.Position,Vector3.new(0,-2000,0),{plr,d})
							if raypart then
								effect("LightWind",Root.CFrame*CFrame.new(0,math.random(-1,1),0)*CFrame.Angles(math.rad(math.random(-20,20)),math.rad(math.random(-180,180)),math.rad(math.random(-20,20)))*CFrame.Angles(0,math.rad(0),0),Vector3.new(0,0,0),wing1.Primary.Color,Enum.Material.Neon,0,.5,Enum.EasingStyle.Sine,Enum.EasingDirection.Out,false,{Size=Vector3.new(175,0,175)},1,Enum.EasingStyle.Sine)
								effect("Sphere",CFrame.new(raypos,raypos+normal)*CFrame.Angles(math.rad(-90),0,0),Vector3.new(theme.PlaybackLoudness/2,0.1,theme.PlaybackLoudness/2),wing1.Primary.Color,Enum.Material.Neon,0,.5,Enum.EasingStyle.Sine,Enum.EasingDirection.Out,false,{Size=Vector3.new(0,0,0)},1,Enum.EasingStyle.Sine)
								if theme.PlaybackLoudness > 100 then
									local randomcf = CFrame.new(math.random(-(theme.PlaybackLoudness/5),(theme.PlaybackLoudness/5)),0,math.random(-(theme.PlaybackLoudness/5),(theme.PlaybackLoudness/5)))
									local cf = CFrame.new(raypos,raypos+normal)*CFrame.Angles(math.rad(-90),0,0)*CFrame.new(0,math.random(10,30),0)
									effect("Cube",CFrame.new(raypos,raypos+normal)*CFrame.Angles(math.rad(-90),0,0)*randomcf,Vector3.new(0,0,0),wing1.Primary.Color,Enum.Material.Neon,0,.45,Enum.EasingStyle.Sine,Enum.EasingDirection.Out,false,{Size=Vector3.new(0.5,math.random(3,13),0.5),CFrame=cf*randomcf},1,Enum.EasingStyle.Sine)
								end
							end
						end
					end
	...
	if wing1 then
		local sea = wing1.Secondary.Color
		local pie = wing1.Primary.Color
		if core then
			if core:FindFirstChild("Base") then
				core.Base.Cyli.Color = sea
				core.Base.Double.Color = pie
				core.Base.Dupli.Color = pie
				core.Base.Penta.Color = pie
				core.Base.Cog.Color = sea
				core.Base.Cyli.Centre.Particle.Color = ColorSequence.new(pie)
				for i,v in pairs(core:GetDescendants()) do
					if v:IsA("Trail") then
						v.Color = ColorSequence.new(pie)
					end
				end
			end
		end
	...
	if w1w ~= nil then
		...
		elseif wingstyle.Value == "HARMONY" then
			...
			height = 18+2*cs(50)
			coredistance = 2
		elseif wingstyle.Value == "NHELV" then
			if Theme.TimePosition < 132.5 or Theme.TimePosition > 154.34 then
				vpower = 0
				local lerp = .7
				w1o,w2o,w3o,w4o,w5o,w6o = 30,35,26,22,40,39
				ap = 5
				p = .3
				w1w.C0 = w1w.C0:lerp(wing1offset*cf(0,0,1)*angles(0,0,math.rad(0+3600*math.cos(sine/75)))*cf(0,-5*cs(25),0),lerp)
				w2w.C0 = w2w.C0:lerp(wing2offset*cf(0,0,1)*angles(0,0,math.rad(90+3600*math.cos(sine/75)))*cf(0,5*cs(25),0),lerp)
				w3w.C0 = w3w.C0:lerp(wing3offset*cf(0,0,1)*angles(0,0,math.rad(180+3600*math.cos(sine/75)))*cf(0,-5*cs(25),0),lerp)
				w4w.C0 = w4w.C0:lerp(wing4offset*cf(0,0,1)*angles(0,0,math.rad(270+3600*math.cos(sine/75)))*cf(0,5*cs(25),0),lerp)
				w5w.C0 = w5w.C0:lerp(wing5offset*cf(0,0,2)*angles(0,0,math.rad(-1800*math.cos(sine/75)))*cf(0,-5*cs(25),0),lerp)
				w6w.C0 = w6w.C0:lerp(wing6offset*cf(0,0,2)*angles(0,0,math.rad(180-1800*math.cos(sine/75)))*cf(0,5*cs(25),0),lerp)
				height = 5+2*cs(45)
				coredistance = 2
			elseif Theme.TimePosition >= 132.5 and Theme.TimePosition < 134.67 then
				vpower = 0
				local lerp = .7
				w1o,w2o,w3o,w4o,w5o,w6o = 30,35,26,22,40,39
				ap = 5
				p = .3
				w1w.C0 = w1w.C0:lerp(wing1offset*angles(math.random(-3600,3600)*math.cos(sine/75),math.random(-3600,3600)*math.cos(sine/75),math.rad(math.random(-3600,3600)*math.cos(sine/75)))*cf(math.random(-100,100)*cs(5),math.random(-100,100)*cs(5),math.random(-100,100)*cs(5)),lerp)
				w2w.C0 = w2w.C0:lerp(wing2offset*angles(math.random(-3600,3600)*math.cos(sine/75),math.random(-3600,3600)*math.cos(sine/75),math.rad(math.random(-3600,3600)*math.cos(sine/75)))*cf(math.random(-100,100)*cs(5),math.random(-100,100)*cs(5),math.random(-100,100)*cs(5)),lerp)
				w3w.C0 = w3w.C0:lerp(wing3offset*angles(math.random(-3600,3600)*math.cos(sine/75),math.random(-3600,3600)*math.cos(sine/75),math.rad(math.random(-3600,3600)*math.cos(sine/75)))*cf(math.random(-100,100)*cs(5),math.random(-100,100)*cs(5),math.random(-100,100)*cs(5)),lerp)
				w4w.C0 = w4w.C0:lerp(wing4offset*angles(math.random(-3600,3600)*math.cos(sine/75),math.random(-3600,3600)*math.cos(sine/75),math.rad(math.random(-3600,3600)*math.cos(sine/75)))*cf(math.random(-100,100)*cs(5),math.random(-100,100)*cs(5),math.random(-100,100)*cs(5)),lerp)
				w5w.C0 = w5w.C0:lerp(wing5offset*angles(math.random(-3600,3600)*math.cos(sine/75),math.random(-3600,3600)*math.cos(sine/75),math.rad(math.random(-3600,3600)*math.cos(sine/75)))*cf(math.random(-100,100)*cs(5),math.random(-100,100)*cs(5),math.random(-100,100)*cs(5)),lerp)
				w6w.C0 = w6w.C0:lerp(wing6offset*angles(math.random(-3600,3600)*math.cos(sine/75),math.random(-3600,3600)*math.cos(sine/75),math.rad(math.random(-3600,3600)*math.cos(sine/75)))*cf(math.random(-100,100)*cs(5),math.random(-100,100)*cs(5),math.random(-100,100)*cs(5)),lerp)
				height = 15+2*cs(45)
				coredistance = 2
			elseif Theme.TimePosition >= 134.67 and Theme.TimePosition < 154.34 then
				vpower = 0
				local lerp = .7
				w1o,w2o,w3o,w4o,w5o,w6o = 30,35,26,22,40,39
				ap = 5
				p = .3
				w1w.C0 = w1w.C0:lerp(wing1offset*angles(math.random(-3600,3600)*math.cos(sine/75),math.random(-3600,3600)*math.cos(sine/75),math.rad(math.random(-3600,3600)*math.cos(sine/75)))*cf(math.random(-20,20)+10*cs(5),math.random(-20,20)+10*cs(5),math.random(-20,20)+10*cs(5)),lerp)
				w2w.C0 = w2w.C0:lerp(wing2offset*angles(math.random(-3600,3600)*math.cos(sine/75),math.random(-3600,3600)*math.cos(sine/75),math.rad(math.random(-3600,3600)*math.cos(sine/75)))*cf(math.random(-20,20)+10*cs(5),math.random(-20,20)+10*cs(5),math.random(-20,20)+10*cs(5)),lerp)
				w3w.C0 = w3w.C0:lerp(wing3offset*angles(math.random(-3600,3600)*math.cos(sine/75),math.random(-3600,3600)*math.cos(sine/75),math.rad(math.random(-3600,3600)*math.cos(sine/75)))*cf(math.random(-20,20)+10*cs(5),math.random(-20,20)+10*cs(5),math.random(-20,20)+10*cs(5)),lerp)
				w4w.C0 = w4w.C0:lerp(wing4offset*angles(math.random(-3600,3600)*math.cos(sine/75),math.random(-3600,3600)*math.cos(sine/75),math.rad(math.random(-3600,3600)*math.cos(sine/75)))*cf(math.random(-20,20)+10*cs(5),math.random(-20,20)+10*cs(5),math.random(-20,20)+10*cs(5)),lerp)
				w5w.C0 = w5w.C0:lerp(wing5offset*angles(math.random(-3600,3600)*math.cos(sine/75),math.random(-3600,3600)*math.cos(sine/75),math.rad(math.random(-3600,3600)*math.cos(sine/75)))*cf(math.random(-20,20)+10*cs(5),math.random(-20,20)+10*cs(5),math.random(-20,20)+10*cs(5)),lerp)
				w6w.C0 = w6w.C0:lerp(wing6offset*angles(math.random(-3600,3600)*math.cos(sine/75),math.random(-3600,3600)*math.cos(sine/75),math.rad(math.random(-3600,3600)*math.cos(sine/75)))*cf(math.random(-20,20)+10*cs(5),math.random(-20,20)+10*cs(5),math.random(-20,20)+10*cs(5)),lerp)
				height = 5+2*cs(45)
				coredistance = 2
			end
		elseif wingstyle.Value == "GLITCH" then
			vpower = 0
			local lerp = .7
			w1o,w2o,w3o,w4o,w5o,w6o = 30,35,26,22,40,39
			ap = 5
			p = .3
			w1w.C0 = w1w.C0:lerp(wing1offset*cf(math.sin(-2,2)*math.sin(sine/10),18+math.sin(-2,2)*math.sin(sine/10),18+math.sin(-2,2)*math.sin(sine/10))*angles(-1800*math.sin(sine),0,math.rad(sine+3600*cs(300)))*cf(0,-2+.5*cs(40),0),lerp)
			w2w.C0 = w2w.C0:lerp(wing2offset*cf(math.sin(-2,2)*math.sin(sine/10),18+math.sin(-2,2)*math.sin(sine/10),18+math.sin(-2,2)*math.sin(sine/10))*angles(-1800*math.sin(sine),0,math.rad(sine+120+3600*cs(300)))*cf(0,-2+.5*cs(40),0),lerp)
			w3w.C0 = w3w.C0:lerp(wing3offset*cf(math.sin(-2,2)*math.sin(sine/10),18+math.sin(-2,2)*math.sin(sine/10),18+math.sin(-2,2)*math.sin(sine/10))*angles(-1800*math.sin(sine),0,math.rad(sine+240+3600*cs(300)))*cf(0,-2+.5*cs(40),0),lerp)
			w4w.C0 = w4w.C0:lerp(wing4offset*cf(math.sin(-2,2)*math.sin(sine/10),18+math.sin(-2,2)*math.sin(sine/10),18+math.sin(-2,2)*math.sin(sine/10))*angles(1800*math.sin(sine),0,math.rad(sine-3600*cs(300)))*cf(0,-2+.5*cs(40),0),lerp)
			w5w.C0 = w5w.C0:lerp(wing5offset*cf(math.sin(-2,2)*math.sin(sine/10),18+math.sin(-2,2)*math.sin(sine/10),18+math.sin(-2,2)*math.sin(sine/10))*angles(1800*math.sin(sine),0,math.rad(sine+120-3600*cs(300)))*cf(0,-2+.5*cs(40),0),lerp)
			w6w.C0 = w6w.C0:lerp(wing6offset*cf(math.sin(-2,2)*math.sin(sine/10),18+math.sin(-2,2)*math.sin(sine/10),18+math.sin(-2,2)*math.sin(sine/10))*angles(1800*math.sin(sine),0,math.rad(sine+240-3600*cs(300)))*cf(0,-2+.5*cs(40),0),lerp)
			height = 0
			coredistance = 2
		elseif wingstyle.Value == "RAVAGEON" then
			vpower = 0
			local lerp = .7
			w1o,w2o,w3o,w4o,w5o,w6o = 30,35,26,22,40,39
			ap = 5
			p = .3
			if theme.TimePosition < 43.8 then
				local PBLoudness = theme.PlaybackLoudness / 75
				w1w.C0 = w1w.C0:lerp(wing1offset*cf(0,0,1)*angles(0,0,math.rad(PBLoudness+3600*math.cos(sine/125)))*cf(0,-12.5*cs(75),0),lerp)
				w2w.C0 = w2w.C0:lerp(wing2offset*cf(0,0,1)*angles(0,0,math.rad((90+PBLoudness)+3600*math.cos(sine/125)))*cf(0,12.5*cs(75),0),lerp)
				w3w.C0 = w3w.C0:lerp(wing3offset*cf(0,0,1)*angles(0,0,math.rad((180+PBLoudness)+3600*math.cos(sine/125)))*cf(0,-12.5*cs(75),0),lerp)
				w4w.C0 = w4w.C0:lerp(wing4offset*cf(0,0,1)*angles(0,0,math.rad((270+PBLoudness)+3600*math.cos(sine/125)))*cf(0,12.5*cs(75),0),lerp)
				w5w.C0 = w5w.C0:lerp(wing5offset*cf(0,0,2)*angles(0,0,math.rad((-1800+PBLoudness)*math.cos(sine/125)))*cf(0,-12.5*cs(75),0),lerp)
				w6w.C0 = w6w.C0:lerp(wing6offset*cf(0,0,2)*angles(0,0,math.rad((180+PBLoudness)-1800*math.cos(sine/125)))*cf(0,12.5*cs(75),0),lerp)
			elseif theme.TimePosition >= 43.8 and theme.TimePosition < 113.26 then
				local PBLoudness = theme.PlaybackLoudness / 50
				w1w.C0 = w1w.C0:lerp(wing1offset*cf(0,0,1)*angles(0,0,math.rad(PBLoudness+3600*math.cos(sine/125)))*cf(0,-15*cs(50),0),lerp)
				w2w.C0 = w2w.C0:lerp(wing2offset*cf(0,0,1)*angles(0,0,math.rad((90+PBLoudness)+3600*math.cos(sine/125)))*cf(0,15*cs(50),0),lerp)
				w3w.C0 = w3w.C0:lerp(wing3offset*cf(0,0,1)*angles(0,0,math.rad((180+PBLoudness)+3600*math.cos(sine/125)))*cf(0,-15*cs(50),0),lerp)
				w4w.C0 = w4w.C0:lerp(wing4offset*cf(0,0,1)*angles(0,0,math.rad((270+PBLoudness)+3600*math.cos(sine/125)))*cf(0,15*cs(50),0),lerp)
				w5w.C0 = w5w.C0:lerp(wing5offset*cf(0,0,2)*angles(0,0,math.rad((-1800+PBLoudness)*math.cos(sine/125)))*cf(0,-15*cs(50),0),lerp)
				w6w.C0 = w6w.C0:lerp(wing6offset*cf(0,0,2)*angles(0,0,math.rad((180+PBLoudness)-1800*math.cos(sine/125)))*cf(0,15*cs(50),0),lerp)
			elseif theme.TimePosition >= 113.26 and theme.TimePosition < 119.5 then
				local PBLoudness = theme.PlaybackLoudness / 25
				w1w.C0 = w1w.C0:lerp(wing1offset*cf(0,0,1)*angles(0,0,math.rad(PBLoudness+3600*math.cos(sine/100)))*cf(0,-17.5*cs(25),0),lerp)
				w2w.C0 = w2w.C0:lerp(wing2offset*cf(0,0,1)*angles(0,0,math.rad((90+PBLoudness)+3600*math.cos(sine/100)))*cf(0,17.5*cs(25),0),lerp)
				w3w.C0 = w3w.C0:lerp(wing3offset*cf(0,0,1)*angles(0,0,math.rad((180+PBLoudness)+3600*math.cos(sine/100)))*cf(0,-17.5*cs(25),0),lerp)
				w4w.C0 = w4w.C0:lerp(wing4offset*cf(0,0,1)*angles(0,0,math.rad((270+PBLoudness)+3600*math.cos(sine/100)))*cf(0,17.5*cs(25),0),lerp)
				w5w.C0 = w5w.C0:lerp(wing5offset*cf(0,0,2)*angles(0,0,math.rad((-1800+PBLoudness)*math.cos(sine/100)))*cf(0,-17.5*cs(25),0),lerp)
				w6w.C0 = w6w.C0:lerp(wing6offset*cf(0,0,2)*angles(0,0,math.rad((180+PBLoudness)-1800*math.cos(sine/100)))*cf(0,17.5*cs(25),0),lerp)
			elseif theme.TimePosition >= 119.5 and theme.TimePosition < 154.5 then
				local PBLoudness = theme.PlaybackLoudness / 50
				w1w.C0 = w1w.C0:lerp(wing1offset*cf(0,0,1)*angles(0,0,math.rad(PBLoudness+3600*math.cos(sine/125)))*cf(0,-15*cs(50),0),lerp)
				w2w.C0 = w2w.C0:lerp(wing2offset*cf(0,0,1)*angles(0,0,math.rad((90+PBLoudness)+3600*math.cos(sine/125)))*cf(0,15*cs(50),0),lerp)
				...
				local PBLoudness = theme.PlaybackLoudness / 42.5
				w1w.C0 = w1w.C0:lerp(wing1offset*cf(0,0,1)*angles(0,0,math.rad(PBLoudness+3600*math.cos(sine/125)))*cf(0,-12.5*cs(75),0),lerp)
				w2w.C0 = w2w.C0:lerp(wing2offset*cf(0,0,1)*angles(0,0,math.rad((90+PBLoudness)+3600*math.cos(sine/125)))*cf(0,12.5*cs(75),0),lerp)
				w3w.C0 = w3w.C0:lerp(wing3offset*cf(0,0,1)*angles(0,0,math.rad((180+PBLoudness)+3600*math.cos(sine/125)))*cf(0,-12.5*cs(75),0),lerp)
				w4w.C0 = w4w.C0:lerp(wing4offset*cf(0,0,1)*angles(0,0,math.rad((270+PBLoudness)+3600*math.cos(sine/125)))*cf(0,12.5*cs(75),0),lerp)
	...
	if Torso:FindFirstChild("Neck") then
		if script.Values.Anim.Value == "STARTUP" then
			g = .05
		...
		if torvel<1 and hitfloor~=nil then
			if script.Values.Anim.Value == "APEX" then
				Anim="Idle"
				local lerp = .07
			...
			elseif script.Values.Anim.Value == "DOWNFALL" then
				Anim="Idle"
				local lerp = .07
				if fury.Value == true then lerp = .3 end
				local h = 30
				...
				LH.C0=LH.C0:Lerp(cf(-1.1+.1*math.sin(sine/g),-.8-.1*math.cos(sine/h)+.05*math.sin(sine/g)-leftlegheight,0.1)*angles(math.rad(-26.6),math.rad(23.2),math.rad(-9.5-5*math.sin(sine/g)))*angles(math.rad(0),math.rad(-90),math.rad(0)),lerp)
				RH.C0=RH.C0:Lerp(cf(0.9+.1*math.sin(sine/g),-1-.1*math.cos(sine/h)-.05*math.sin(sine/g)-rightlegheight,-0.3)*angles(math.rad(-1.8),math.rad(-20.1),math.rad(17.4-5*math.sin(sine/g)))*angles(math.rad(0),math.rad(90),math.rad(0)),lerp)
			elseif script.Values.Anim.Value == "EXASPERATION" then
				Anim="Idle"
				local lerp = .07
				if fury.Value == true then lerp = .3 end
			...
			elseif script.Values.Anim.Value == "PERDURANCE" then
				local lerp = .07
				if fury.Value == true then lerp = .3 end
			...
			elseif script.Values.Anim.Value == "REPRESSION" then
				Anim="Idle"
				local lerp = .07
				if fury.Value == true then lerp = .3 end
			...
			elseif script.Values.Anim.Value == "ADMISSION" then
				Anim="Idle"
				local lerp = .07
				if fury.Value == true then lerp = .3 end
				RootJoint.C0=RootJoint.C0:Lerp(RootCF*cf(.2*math.cos(sine/40),0,-0.2+.1*math.cos(sine/30))*angles(math.rad(19.2),math.rad(0+5*math.cos(sine/40)),math.rad(-30.1)),lerp)
				...
				LH.C0=LH.C0:Lerp(cf(-1.1,-0.8+.1*math.cos(sine/40)-.1*math.cos(sine/30)-leftlegheight,0)*angles(math.rad(26.2),math.rad(37.7),math.rad(-19.4))*angles(math.rad(0),math.rad(-90),math.rad(0)),lerp)
				RH.C0=RH.C0:Lerp(cf(0.8,-1.2-.1*math.cos(sine/40)-.1*math.cos(sine/30)-rightlegheight,0.1)*angles(math.rad(0),math.rad(-25.5),math.rad(12))*angles(math.rad(0),math.rad(90),math.rad(0)),lerp)
			elseif script.Values.Anim.Value == "EXECUTION" then
				Anim="Idle"
				local lerp = .07
				if fury.Value == true then lerp = .3 end
			...
			elseif script.Values.Anim.Value == "STIGMA" then
				Anim="Idle"
				local lerp = .07
				if fury.Value == true then lerp = .3 end
				RootJoint.C0=RootJoint.C0:Lerp(RootCF*cf(1*cs(70),1*cs(80),5-1*math.cos(sine/50))*angles(math.rad(-10+5*math.cos(sine/35)),math.rad(-30+5*math.cos(sine/37)),math.rad(67.6+5*math.cos(sine/42))),lerp)
				...
				RH.C0=RH.C0:Lerp(cf(1,0+.25*math.cos(sine/43),-0.7)*angles(math.rad(-19+5*math.cos(sine/30)),math.rad(-16.2+5*math.cos(sine/38)),math.rad(7.5+5*math.cos(sine/45)))*angles(math.rad(0),math.rad(90),math.rad(0)),lerp)
			elseif script.Values.Anim.Value == "VINDICTIVE" then
				Anim="Idle"
				local lerp = .07
				if fury.Value == true then lerp = .3 end
			...
			elseif script.Values.Anim.Value == "NIHIL" then
				Anim="Idle"
				local lerp = .07
				if fury.Value == true then lerp = .3 end
			...
			elseif script.Values.Anim.Value == "ANARCHY" then
				Anim="Idle"
				local lerp = .07
				if fury.Value == true then lerp = .3 end
			...
			elseif script.Values.Anim.Value == "TEMPEST" then
				Anim="Idle"
				local lerp = .07
				if fury.Value == true then lerp = .3 end
			...
			elseif script.Values.Anim.Value == "INFERNUM" then
				Anim="Idle"
				local lerp = .05
				if fury.Value == true then lerp = .3 end
			...
			elseif script.Values.Anim.Value == "VIRTUE" then
				Anim="Idle"
				local lerp = .07
				if fury.Value == true then lerp = .3 end
			...
			elseif script.Values.Anim.Value == "ABSOLUTION" then
				Anim="Idle"
				local lerp = .07
				if fury.Value == true then lerp = .3 end
			...
			elseif script.Values.Anim.Value == "RETENTION" then
				Anim="Idle"
				local lerp = .07
				if fury.Value == true then lerp = .3 end
				RootJoint.C0=RootJoint.C0:Lerp(RootCF*cf(1*cs(65),1*cs(70),height)*angles(0,math.rad(10*math.sin(sine/90)),0)*angles(math.rad(12.2+5*cs(48)),math.rad(5*cs(50)),math.rad(-45.2+5*cs(40))),lerp)
				...
				LH.C0=LH.C0:Lerp(cf(-0.9,-0.4+.2*cs(46),-1.1)*angles(math.rad(-54.9+10*cs(42)),math.rad(13.1),math.rad(-3.2))*angles(math.rad(0),math.rad(-90),math.rad(0)),lerp)
				RH.C0=RH.C0:Lerp(cf(0.9,-1+.1*cs(49),-0.3)*angles(math.rad(-31.5),math.rad(-35.8),math.rad(0.6))*angles(math.rad(0),math.rad(90),math.rad(0)),lerp)
			elseif script.Values.Anim.Value == "DISSONANCE" then
				Anim="Idle"
				local lerp = .05
				if fury.Value == true then lerp = .3 end
			...
			elseif script.Values.Anim.Value == "WARPSPEED" then
				Anim = "Idle"
				local lerp = .07
				if fury.Value == true then lerp = .3 end
			...
			elseif script.Values.Anim.Value == "APOCALYPSE" then
				Anim = "Idle"
				local lerp = .07
				if fury.Value == true then lerp = .3 end
			...
			elseif script.Values.Anim.Value == "INTRICACY" then
				Anim = "idle"
				local lerp = .07
				if fury.Value == true then lerp = .3 end
			...
			elseif script.Values.Anim.Value == "SAGITTARIUS" then
				Anim="Idle"
				local lerp = .07
				if fury.Value == true then lerp = .3 end
			...
			elseif script.Values.Anim.Value == "HARMONY" then
				Anim="Idle"
				local lerp = .07
				if fury.Value == true then lerp = .3 end
			...
			elseif script.Values.Anim.Value == "NHELV" then
				Anim="Idle"
				if Theme.TimePosition < 131 or Theme.TimePosition > 154.34 then
					local lerp = .3
					RootJoint.C0=RootJoint.C0:Lerp(RootCF*cf(math.random(-1000,1000)/1000,math.random(-1000,1000)/1000,5+2*cs(40))*angles(math.rad(30+math.random(-10,10)),math.rad(0+math.random(-10,10)),math.rad(0+math.random(-10,10))),lerp)
					Torso.Neck.C0=Torso.Neck.C0:Lerp(necko*furyshake()*cf(0,0,0)*angles(math.rad(25+math.random(-10,10)),math.rad(5+math.random(-10,10)),math.rad(5+math.random(-10,10))),lerp)
					LS.C0 = LS.C0:lerp(cf(-1.5,0.3+.1 * math.cos(sine/40),0.2)*furyshake()*angles(math.rad(8.3),math.rad(25.8),math.rad(-22)),lerp)
					RS.C0 = RS.C0:lerp(cf(1.3,0.4+.1 * math.cos(sine/40),-0.9)*furyshake()*angles(math.rad(158.4),math.rad(-32.8),math.rad(-57)),lerp)
					LH.C0=LH.C0:Lerp(cf(-0.9,-0.8,0.1)*angles(math.rad(10+5*cs(59)),math.rad(14.5+5*cs(57)),math.rad(-2.6))*angles(math.rad(0),math.rad(-90),math.rad(0)),lerp)
					RH.C0=RH.C0:Lerp(cf(0.7,-0.4+.3*cs(55),-1)*angles(math.rad(-44.9+10*cs(58)),math.rad(-15.2+5*cs(47)),math.rad(13.7+5*cs(44)))*angles(math.rad(0),math.rad(90),math.rad(0)),lerp)
				elseif Theme.TimePosition >= 131 and Theme.TimePosition < 132.5 then
					local lerp = .47
					RootJoint.C0=RootJoint.C0:Lerp(RootCF*cf(math.random(-1000,1000)/1000,math.random(-1000,1000)/1000,5+2*cs(40))*angles(math.rad(30+math.random(-10,10)),math.rad(0+math.random(-10,10)),math.rad(0+math.random(-10,10))),lerp)
					Torso.Neck.C0=Torso.Neck.C0:Lerp(necko*furyshake()*cf(0,0,0)*angles(math.rad(35+math.random(-15,15)),math.rad(5+math.random(-15,15)),math.rad(5+math.random(-15,15))),lerp)
					LS.C0 = LS.C0:lerp(cf(-1.3,0.4+.1 * math.cos(sine/40),-0.86)*furyshake()*angles(math.rad(153.4),math.rad(25.8),math.rad(49)),lerp)
					RS.C0 = RS.C0:lerp(cf(1.3,0.4+.1 * math.cos(sine/40),-0.9)*furyshake()*angles(math.rad(158.4),math.rad(-32.8),math.rad(-57)),lerp)
					LH.C0=LH.C0:Lerp(cf(-0.9,-0.8,0.1)*angles(math.rad(10+5*cs(59)),math.rad(14.5+5*cs(57)),math.rad(-2.6))*angles(math.rad(0),math.rad(-90),math.rad(0)),lerp)
					RH.C0=RH.C0:Lerp(cf(0.7,-0.4+.3*cs(55),-1)*angles(math.rad(-44.9+10*cs(58)),math.rad(-15.2+5*cs(47)),math.rad(13.7+5*cs(44)))*angles(math.rad(0),math.rad(90),math.rad(0)),lerp)
				elseif Theme.TimePosition >= 132.5 and Theme.TimePosition < 134.67 then
					local lerp = .53
					RootJoint.C0=RootJoint.C0:Lerp(RootCF*cf(math.random(-1000,1000)/1000,math.random(-1000,1000)/1000,15+2*cs(40))*angles(math.rad(-25+5*cs(55)),math.rad(5*cs(58)),math.rad(5*cs(45))),lerp)
					Torso.Neck.C0=Torso.Neck.C0:Lerp(necko*furyshake()*cf(0,0,0)*angles(math.rad(-22.7+5*cs(54)),math.rad(5*cs(48)),math.rad(5*cs(51))),lerp)
					LS.C0=LS.C0:Lerp(cf(-1.5,1,0)*furyshake()*angles(math.rad(-25.4),math.rad(0),math.rad(-90+10*math.sin(sine/-50))),lerp)
					RS.C0=RS.C0:Lerp(cf(1.5,1,0)*furyshake()*angles(math.rad(-25.4),math.rad(0),math.rad(90-10*math.sin(sine/-50))),lerp)
					LH.C0=LH.C0:Lerp(cf(-0.9,-0.8,0.1)*angles(math.rad(-25.6+5*cs(59)),math.rad(14.5+5*cs(57)),math.rad(-2.6))*angles(math.rad(0),math.rad(-90),math.rad(0)),lerp)
				...
				elseif Theme.TimePosition >= 134.67 and Theme.TimePosition < 154.34 then
					local lerp = .3
					RootJoint.C0=RootJoint.C0:Lerp(RootCF*cf(math.random(-1000,1000)/1000,math.random(-1000,1000)/1000,5+2*cs(40))*angles(math.rad(30+math.random(-10,10)),math.rad(180+math.random(-10,10)),math.rad(0+math.random(-10,10))),lerp)
					Torso.Neck.C0=Torso.Neck.C0:Lerp(necko*furyshake()*cf(0,0,0)*angles(math.rad(35+math.random(-15,15)),math.rad(5+math.random(-15,15)),math.rad(5+math.random(-15,15))),lerp)
					LS.C0 = LS.C0:lerp(cf(-1.3,0.4+.1 * math.cos(sine/40),-0.86)*furyshake()*angles(math.rad(153.4),math.rad(25.8),math.rad(49)),lerp)
					RS.C0 = RS.C0:lerp(cf(1.3,0.4+.1 * math.cos(sine/40),-0.9)*furyshake()*angles(math.rad(158.4),math.rad(-32.8),math.rad(-57)),lerp)
					LH.C0=LH.C0:Lerp(cf(-0.9,-0.8,0.1)*angles(math.rad(10+5*cs(59)),math.rad(14.5+5*cs(57)),math.rad(-2.6))*angles(math.rad(0),math.rad(-90),math.rad(0)),lerp)
			...
			elseif script.Values.Anim.Value == "GLITCH" then
				Anim = "Idle"
				local lerp = .4
				if fury.Value == true then lerp = .7 end
				RootJoint.C0=RootJoint.C0:Lerp(cf(0,-0.5+.1*cs(32),0)*angles(math.rad(-43.5+5*math.sin(sine/32)),math.rad(0),math.rad(0))*RootCF,lerp)
				Torso.Neck.C0=Torso.Neck.C0:Lerp(necko*furyshake()*cf(0,0,0)*angles(math.rad(15+math.random(-15,15)),math.rad(5+math.random(-15,15)),math.rad(5+math.random(-15,15))),lerp)
				LS.C0 = LS.C0:lerp(cf(-1.4,0.7+.1 * math.cos(sine/40),-0.4)*furyshake()*angles(math.rad(153.4),math.rad(32.8),math.rad(57)),lerp)
				RS.C0 = RS.C0:lerp(cf(1.4,0.7+.1 * math.cos(sine/40),-0.4)*furyshake()*angles(math.rad(158.4),math.rad(-32.8),math.rad(-57)),lerp)
				RH.C0 = RH.C0:Lerp(cf(1.2,-.4+.05*cs(32)-.05*math.sin(sine/32)-(rightlegheight*2),-0.4-.05*cs(32)+.05*math.sin(sine/32))*angles(math.rad(48+7*math.cos(sine/30)),math.rad(-16.2-5*math.cos(sine/38)),math.rad(7.5+5*math.cos(sine/45)))*angles(math.rad(0),math.rad(90),math.rad(0)),lerp)
				LH.C0 = LH.C0:Lerp(cf(-0.8,-0.7-.05*cs(32)-.05*math.sin(sine/32)-leftlegheight,-0.6-.05*cs(32)+.05*math.sin(sine/32))*angles(math.rad(6.6-5*math.sin(sine/32)),math.rad(6.3),math.rad(-7.4))*angles(math.rad(0),math.rad(-90),math.rad(0)),lerp)
			elseif script.Values.Anim.Value == "RAVAGEON" then
				Anim = "Idle"
				local lerp = .47
				if fury.Value == true then lerp = .7 end
				if theme.TimePosition < 111 or theme.TimePosition >= 119.5 then
					RootJoint.C0=RootJoint.C0:Lerp(RootCF*cf(math.random(-100,100)/1000,math.random(-100,100)/1000,5+2*cs(40))*angles(math.rad(30+math.random(-3,3)),math.rad(0+math.random(-3,3)),math.rad(0+math.random(-3,3))),lerp)
					Torso.Neck.C0=Torso.Neck.C0:Lerp(necko*furyshake()*cf(0,0,0)*angles(math.rad(30+math.random(-7,7)),math.rad(math.random(-7,7)),math.rad(math.random(-7,7))),lerp)
					LS.C0 = LS.C0:lerp(cf(-1.5,-.35+.1 * math.cos(sine/40),-0.2)*furyshake()*angles(math.rad(162.3+18.6*cs(40)),math.rad(25.8),math.rad(-17.5-3*cs(40))),lerp)
					RS.C0 = RS.C0:lerp(cf(1.5,-.35+.1 * math.cos(sine/40),-0.2)*furyshake()*angles(math.rad(162.3+18.6*cs(40)),math.rad(-25.8),math.rad(17.5+3*cs(40))),lerp)
					LH.C0=LH.C0:Lerp(cf(-0.9,-0.8,-.37)*angles(math.rad(-20+5*cs(59)),math.rad(40+5*cs(57)),math.rad(-2.6))*angles(math.rad(0),math.rad(-90),math.rad(0)),lerp)
					RH.C0=RH.C0:Lerp(cf(0.7,-0.4+.3*cs(55),-1)*angles(math.rad(-44.9+10*cs(58)),math.rad(-15.2+5*cs(47)),math.rad(13.7+5*cs(44)))*angles(math.rad(0),math.rad(90),math.rad(0)),lerp)
				elseif theme.TimePosition >= 111 and theme.TimePosition < 113.26 then
					RootJoint.C0=RootJoint.C0:Lerp(RootCF*cf(math.random(-200,200)/1000,math.random(-200,200)/1000,5+2*cs(40))*angles(math.rad(40+math.random(-6,6)),math.rad(0+math.random(-6,6)),math.rad(0+math.random(-6,6))),lerp)
					Torso.Neck.C0=Torso.Neck.C0:Lerp(necko*furyshake()*cf(0,0,0)*angles(math.rad(30+math.random(-7,7)),math.rad(math.random(-7,7)),math.rad(math.random(-7,7))),lerp)
					LS.C0 = LS.C0:lerp(cf(-1.35,.6+.1 * math.cos(sine/40),-0.8)*furyshake()*angles(math.rad(150),math.rad(20.8),math.rad(65)),lerp)
					RS.C0 = RS.C0:lerp(cf(1.35,.6+.1 * math.cos(sine/40),-0.8)*furyshake()*angles(math.rad(150),math.rad(-20.8),math.rad(-65)),lerp)
					LH.C0=LH.C0:Lerp(cf(-0.9,-0.8,-.37)*angles(math.rad(-20+5*cs(59)),math.rad(40+5*cs(57)),math.rad(-2.6))*angles(math.rad(0),math.rad(-90),math.rad(0)),lerp)
					RH.C0=RH.C0:Lerp(cf(0.7,-0.4+.3*cs(55),-1)*angles(math.rad(-44.9+10*cs(58)),math.rad(-15.2+5*cs(47)),math.rad(13.7+5*cs(44)))*angles(math.rad(0),math.rad(90),math.rad(0)),lerp)
				elseif theme.TimePosition >= 113.26 and theme.TimePosition < 119.5 then
					RootJoint.C0=RootJoint.C0:Lerp(RootCF*cf(math.random(-1500,1500)/1000,math.random(-1500,1500)/1000,(math.random(-500,500)/1000)+5+2*cs(40))*angles(math.rad(40+math.random(-13,13)),math.rad(0+math.random(-13,13)),math.rad(0+math.random(-13,13))),math.random(500,850)/1000)
					Torso.Neck.C0=Torso.Neck.C0:Lerp(necko*furyshake()*cf(0,0,0)*angles(math.rad(-40+math.random(-7,7)),math.rad(math.random(-7,7)),math.rad(math.random(-7,7))),lerp)
					LS.C0 = LS.C0:lerp(cf(-1.5,.4+.1 * math.cos(sine/40),-0.1)*furyshake()*angles(math.rad(-20.3+5*cs(40)),math.rad(-25.8),math.rad(-26.3-3*cs(40))),lerp)
					RS.C0 = RS.C0:lerp(cf(1.5,.4+.1 * math.cos(sine/40),-0.1)*furyshake()*angles(math.rad(-20.3+5*cs(40)),math.rad(25.8),math.rad(26.3+3*cs(40))),lerp)
					LH.C0=LH.C0:Lerp(cf(-0.9,-0.8,-.37)*angles(math.rad(-20+5*cs(59)),math.rad(40+5*cs(57)),math.rad(-2.6))*angles(math.rad(0),math.rad(-90),math.rad(0)),lerp)
					RH.C0=RH.C0:Lerp(cf(0.8,-0.4+.3*cs(55),-1)*angles(math.rad(-27+10*cs(58)),math.rad(-15.2+5*cs(47)),math.rad(13.7+5*cs(44)))*angles(math.rad(0),math.rad(90),math.rad(0)),lerp)
				end
			end
		end
		
		if script.Values.Anim.Value == "SWITCH START" then
			local lerp = .4
			if height == 0 then
		...
		elseif script.Values.Anim.Value == "SWITCH" then
			local lerp = .1
			if height == 0 then
		...
		elseif script.Values.Anim.Value == "SWING" then
			lerp = .4
		...
		elseif script.Values.Anim.Value == "GRAB" then
			lerp = .6
		...
		elseif script.Values.Anim.Value == "SLAM" then
			lerp = .9
		...
		elseif script.Values.Anim.Value == "REPRESSIONBOMBCHARGE" then
			lerp = .1
		...
		elseif script.Values.Anim.Value == "REPRESSIONBOMBDETONATE" then
			lerp = .5
			RootJoint.C0=RootJoint.C0:Lerp(RootCF*cf(0,-0.2,0)*angles(math.rad(19.2),math.rad(0),math.rad(-30.1)),lerp)
			Torso.Neck.C0=Torso.Neck.C0:Lerp(necko*cf(0.3,0,0.1)*angles(math.rad(10.4),math.rad(14),math.rad(24.2)),lerp)
			LS.C0=LS.C0:Lerp(cf(-1.1,0.5,-0.6)*angles(math.rad(-150.1),math.rad(-70.7),math.rad(121.3)),lerp)
			...
			LH.C0=LH.C0:Lerp(cf(-1.1,-0.8,0)*angles(math.rad(26.2),math.rad(37.7),math.rad(-19.4))*angles(math.rad(0),math.rad(-90),math.rad(0)),lerp)
			RH.C0=RH.C0:Lerp(cf(0.8,-1.2,0.1)*angles(math.rad(0),math.rad(-25.5),math.rad(12))*angles(math.rad(0),math.rad(90),math.rad(0)),lerp)
		elseif script.Values.Anim.Value == "REPRESSIONCIRCLESSTART" then
			lerp = .2
		...
		elseif script.Values.Anim.Value == "REPRESSIONCIRCLESEND" then
			lerp = .5
		...
		elseif script.Values.Anim.Value == "TELEPORT" then
			lerp = .08
			local addition = 0
		...
		elseif script.Values.Anim.Value == "PowerFall" then
			lerp = .05
		...
		elseif script.Values.Anim.Value == "PowerHit" then
			lerp = .99
		...
		elseif script.Values.Anim.Value == "ExecutionGrab" then
			lerp = .5
		...
		elseif script.Values.Anim.Value == "ExecutionThrow" then
			lerp = .5
		...
		elseif script.Values.Anim.Value == "ExecutionCharge" then
			lerp = .5
		...
		elseif script.Values.Anim.Value == "ExecutionSlam" then
			lerp = .9
			...
			LH.C0=LH.C0:Lerp(cf(-0.8,-0.9,-1.1)*angles(math.rad(-73.8),math.rad(13.9),math.rad(-11.5))*angles(math.rad(0),math.rad(-90),math.rad(0)),lerp)
			RH.C0=RH.C0:Lerp(cf(1,-0.9,0)*angles(math.rad(-30.2),math.rad(-9.9),math.rad(5.7))*angles(math.rad(0),math.rad(90),math.rad(0)),lerp)
		elseif script.Values.Anim.Value == "RESTING" then
			lerp = .05
		...
		elseif script.Values.Anim.Value == "FLAMETHROWERSTART" then
			lerp = .1
		...
		elseif script.Values.Anim.Value == "FLAMETHROWEREND" then
			lerp = .7
		...
		elseif script.Values.Anim.Value == "HADOUKENSTART" then
			lerp = .3
		...
		elseif script.Values.Anim.Value == "HADOUKENEND" then
			lerp = .8
			...
			RH.C0=RH.C0:Lerp(cf(0.9,-1.3,0.1)*angles(math.rad(12.4),math.rad(-29.9),math.rad(16.5))*angles(math.rad(0),math.rad(90),math.rad(0)),lerp)
			LH.C0=LH.C0:Lerp(cf(-1,-1,0.4)*angles(math.rad(19.6),math.rad(63.7),math.rad(-12.8))*angles(math.rad(0),math.rad(-90),math.rad(0)),lerp)
		elseif script.Values.Anim.Value == "VINDICTIVE Z" then
			local lerp = .05
		...
		elseif script.Values.Anim.Value == "VINDICTIVE Z END" then
			local lerp = .8
		...
		elseif script.Values.Anim.Value == "Ground Stomp 1" then
			local lerp = .1
		...
		elseif script.Values.Anim.Value == "Ground Stomp 2" then
			local lerp = .99
		...
		elseif script.Values.Anim.Value == "Mouse1Start" then
			local lerp = .2
		...
		elseif script.Values.Anim.Value == "Mouse1End" then
			local lerp = .7
		...
		elseif script.Values.Anim.Value == "Healing Burst Start" then
			local lerp = .1
		...
		elseif script.Values.Anim.Value == "Healing Burst End" then
			local lerp = .7
		...
		elseif script.Values.Anim.Value == "Execution Z 1" then
			local lerp = .1
		...
		elseif script.Values.Anim.Value == "Execution Z 2" then
			local lerp = .1
			RootJoint.C0=RootJoint.C0:Lerp(cf(1*cs(80),5-1*math.cos(sine/50),1*cs(70))*angles(math.rad(75.1),math.rad(55.3),math.rad(-52.3))*RootCF,lerp)
			Torso.Neck.C0=Torso.Neck.C0:Lerp(cf(0.2,0.2,0.6)*angles(math.rad(-11.3),math.rad(-64.8),math.rad(29.1))*necko,lerp)
			...
			RH.C0=RH.C0:Lerp(cf(1,0,-0.7)*angles(math.rad(-19),math.rad(-16.2),math.rad(7.5))*angles(math.rad(0),math.rad(90),math.rad(0)),lerp)
			LH.C0=LH.C0:Lerp(cf(-0.8,-0.9,0)*angles(math.rad(-46.8),math.rad(9.3),math.rad(-10.3))*angles(math.rad(0),math.rad(-90),math.rad(0)),lerp)
		elseif script.Values.Anim.Value == "ApexRun" then
			local lerp = .3
			local m = 6
			local n = m/2
		...
		elseif script.Values.Anim.Value == "Coin Toss 1" then
			local lerp = .3
		...
		elseif script.Values.Anim.Value == "Coin Toss 2" then
			local lerp = .3
...
local NetlessOffset = CFrame.new(-6, 0, 0)
    * CFrame.Angles(math.rad(0), math.rad(90), math.rad(0))
    * CFrame.Angles(math.rad(-40), math.rad(0), math.rad(0))
--    * CFrame.fromAxisAngle(Vector3.new(1,0,0), math.rad(0))

local NetlessConfig = {
    { TextureID = "5316471565", WingName = "Wing1" },
    { TextureID = "5278777022", WingName = "Wing2" },
    { TextureID = "5316510551", WingName = "Wing3" },
    { TextureID = "5692006383", WingName = "Wing4" },
    { TextureID = "5268538095", WingName = "Wing5" },
    { TextureID = "5254572443", WingName = "Wing6" },
}

-- [防掉落+防抖] 纯物理约束方案：
-- 位置：直接设 CFrame 精确跟随；AlignPosition 仅作防掉落兜底（位置已精确时不发力）
-- 不再直接设置 Handle.CFrame，避免"脚本设置 vs 物理约束"的竞争抖动
local function setupSwordSafety(handle)
    ...
    -- 位置约束：Position 是世界坐标
    local alignPos = handle:FindFirstChild("SwordAlignPos")
    if not alignPos then
        ...
        pcall(function() alignPos.RigidityEnabled = true end) -- [防抖] 刚性约束
...
task.spawn(function()
    ...
    local function findTargetPart(config)
        local wing = Character:FindFirstChild(config.WingName)
        if wing then
            local secondary = wing:FindFirstChild("Secondary", true)
    ...
    local function findSwordHandle(textureId)
        for _, acc in pairs(Character:GetChildren()) do
            if acc:IsA("Accessory") then
                local handle = acc:FindFirstChild("Handle")
                if handle then
                    local mesh = handle:FindFirstChildOfClass("SpecialMesh")
                    local currentId = handle:IsA("MeshPart") and handle.TextureID or (mesh and mesh.TextureId or "")
                    if string.find(tostring(currentId), textureId) then
                        return handle
                    end
                end
            end
    ...
    while #ActiveSyncList < #NetlessConfig do
        for _, config in pairs(NetlessConfig) do
            if not config.Activated then
                ...
                if targetPart then
                    local handle = findSwordHandle(config.TextureID)
                    if handle then
                        handle:BreakJoints()
                        setupSwordSafety(handle)   -- [防掉落+防抖] 绑定物理约束
                        config.Activated = true
                        table.insert(ActiveSyncList, {
                            HatHandle = handle,
                            TargetPart = targetPart,
                            Offset = NetlessOffset,
                        })
                    end
                end
            end
    ...
    RunService.Heartbeat:Connect(function()
        for _, syncData in pairs(ActiveSyncList) do
            ...
            if hat and hat.Parent and target and target.Parent then
                ...
                -- [精确跟随] 直接设 CFrame：位置+旋转零滞后（约束追不上快速摆动的翅膀，所以不用它追）
                hat.CFrame = targetCF
                -- [防掉落兜底] 约束目标同步：正常时位置已精确、约束不发力；被服务器干扰时拉回
                local saPos = hat:FindFirstChild("SwordAlignPos")
